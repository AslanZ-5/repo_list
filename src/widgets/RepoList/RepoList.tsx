...
import { GET_REPOSITORIES, GET_CURRENT_USER_REPOS } from "../../const/gql";
...
const RepoList = () => {
  ...
  const repositryQuery = delayedQuery
    ? GET_REPOSITORIES
  ...
  const dataArr2: NodeT[] = delayedQuery
    ? data?.search?.edges.map((i: { __type: string; node: object }) => i.node)
    : data?.viewer?.repositories?.nodes;
  const pageinfo: PageInfoT = delayedQuery
    ? data?.search?.pageInfo
    : data?.viewer?.repositories?.pageInfo;
  useEffect(() => {
    getRepositories();
  }, [delayedQuery, getRepositories]);
